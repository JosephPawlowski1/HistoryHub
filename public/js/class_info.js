...
window.onload = function() {
  ...
  //    console.log(classCommentObj);
    ...
    let initListOfTasks = () => {
      ...
      classCommentsObjs.forEach(classCommentsObjs => {
        createTaskCard(classCommentsObjs);
      });
    };
...
console.log("DID I REACH BITCH?")

$("commentEventBTN").on("click", function(event) {
  alert("GOT HERE");
  document.location.href = "createClassComment.html";
});

$("#deleteClassBTN").on("click", function(event) {
  key = sessionStorage.getItem("eventKey");
  var learnMoreNum = sessionStorage.getItem("learnMoreNum");
  var array = key.split(",");
  var num = Number(learnMoreNum);
